...
async function boot(){
 try{
  const response=await fetch("/data/flpr-data.json?v=1.0.0-prod",{cache:"no-store"});
...
function route(){if(!DATA)return;let h=location.hash.slice(1)||"home";if(h.startsWith("player/"))return playerPage(h.split("/")[1]);({home,rankings,changes:ratingChanges,players:playersPage,awards,import:importPage,methodology}[h]||home)();navActive(h)}
...
function home(){
 ...
 <div class="sectionHead" style="margin-top:45px"><div><h2>Live Rating Change</h2><p>Official before-and-after movement from the latest approved rating snapshot.</p></div><a class="btn" href="#changes">Open movement center →</a></div>
 <div class="changeStrip">${p.slice(0,3).map(x=>changeCard(x,true)).join("")}</div>
 <div class="baselineNotice"><b>Production baseline active.</b> The next approved tournament will generate the first live rating movements.</div>
